        ...
        >
            ...
            <div className="absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center z-10 p-6 sm:p-8 md:p-10">
                <div className="font-['Gagalin-Regular']">
                    <h2 className="text-white font-bold text-xl sm:text-5xl mb-4  text-center">
                        {title.split("\n").map((line, index) => (
                    ...
                    <p className="font-semibold text-xs sm:text-xl md:text-2xl text-left  sm:pr-32">
                        {text.split("\n").map((line, index) => (
                            <React.Fragment key={index}>
                                {line}
                                <br />
            ...
            <img
                ...
                alt={imageAlt}
                loading='lazy'
                className="absolute -bottom-0 -right-4 w-20 h-20 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-64 lg:h-64 object-contain z-5" // Ensure it’s contained and doesn’t overlap
            />
        </div>
    );
};
